...
def eval_KITTI(model, config, use_icp):
    dset = KITTIDataset(root='./datasets/KITTI',
                        split='test',
...
if __name__ == '__main__':
    ...
    config_path = './GPINets-master/snapshot/GPINets_KITTI_xxxx/config.json'
    ...
    for i in range(50, 101):
        miss = model.load_state_dict(torch.load(
            './GPINets-master/snapshot/GPINets_KITTI_xxxx/models/model_best.pkl'),
                                     strict=False)
        print(miss)
        model.eval()
