...
class KITTIDataset(data.Dataset):
    def __init__(self,
                 root=f'/media/lixiaohan/下载/huohu/PointDSC-master/data/KITTI/',
                 split='train',
                 descriptor='fpfh',
                 in_dim=6,
    ...
    def __getitem__(self, index):
        ...
        src_topk = np.argsort(distance, axis=1)[:, :2].astype(np.int32)  # shape: [N_src, k]
        # 2. 对每个 tgt 找到 top-k 的 src
        tgt_topk = np.argsort(distance, axis=0)[:2, :].astype(np.int32)  # shape: [k, N_tgt]
        # 3. 建立候选匹配对
        mutual_corr = []
...
if __name__ == "__main__":
    dset = KITTIDataset(
        root='/media/lixiaohan/下载/huohu/PointDSC-master/data/KITTI/',
        split='test',
        descriptor='fpfh',
        num_node=5000,
        use_mutual=False,
