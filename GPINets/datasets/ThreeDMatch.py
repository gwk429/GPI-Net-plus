...
class ThreeDMatchTrainVal(data.Dataset):
    def __init__(self,
        ...
        OVERLAP_RATIO = 0.3
        DATA_FILES = {
            'train': 'F:/gwk/PG-Net-master/misc/split/train_3dmatch.txt',
            'val': 'F:/gwk/PG-Net-master/misc/split/val_3dmatch.txt',
    ...
    def __getitem__(self, index):
        ...
        source_idx = np.argmin(distance, axis=1).astype(np.int32)
        ...
        if self.use_mutual:
            ...
            corr1 = np.concatenate([np.where(mutual_nearest == 1)[0][:, None], source_idx[mutual_nearest][:, None]], axis=-1)
        else:
            corr1 = np.concatenate([np.arange(source_idx.shape[0])[:, None], source_idx[:, None]], axis=-1)
        if len(corr1) < 10:
            # skip pairs with too few correspondences.
            return self.__getitem__(int(np.random.choice(self.__len__(), 1)))

        # 1. 对每个 src 找到 top-k 的 tgt
        src_topk = np.argsort(distance, axis=1)[:, :5].astype(np.int32)  # shape: [N_src, k]
        # 2. 对每个 tgt 找到 top-k 的 src
        tgt_topk = np.argsort(distance, axis=0)[:5, :].astype(np.int32)  # shape: [k, N_tgt]
        # 3. 建立候选匹配对
        mutual_corr = []
        for src_idx in range(src_desc.shape[0]):
            for rank in range(5):
                tgt_idx = src_topk[src_idx, rank]
                # 检查 src_idx 是否也在 tgt_idx 的前k邻居中
                if src_idx in tgt_topk[:, tgt_idx]:
                    mutual_corr.append([src_idx, tgt_idx])
        # 4. 转换为 numpy 数组、合并、去重
        corr = np.array(mutual_corr, dtype=np.int32)  # shape: [N_corr, 2]
        corr1 = corr1.astype(np.int32)
        corr = np.vstack((corr1, corr))
        corr = np.unique(corr, axis=0)
...
class ThreeDLOMatchTest(data.Dataset):
    def __init__(self,
        ...
        with open('F:\\gwk\\PG-Net-master\\misc\\3DLoMatch.pkl', 'rb') as f:
    ...
    def __getitem__(self, item):
        ...
        if self.use_mutual:
            target_idx = np.argmin(distance, axis=0)
            mutual_nearest = (target_idx[source_idx] == np.arange(source_idx.shape[0]))
            corr1 = np.concatenate([np.where(mutual_nearest == 1)[0][:, None], source_idx[mutual_nearest][:, None]],
                                  axis=-1)
        else:
            corr1 = np.concatenate([np.arange(source_idx.shape[0])[:, None], source_idx[:, None]], axis=-1)
